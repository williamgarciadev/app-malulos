...
- [Despliegue de Malulos POS (Docker + NPM)](#despliegue-de-malulos-pos-docker--npm)
...
## Despliegue de Malulos POS (Docker + NPM)

Objetivo: levantar web + API + PostgreSQL y publicarlo con NPM.

Paso 1: Crea la red compartida para NPM
- sudo docker network create malulos_net
- sudo docker network connect malulos_net npm

Paso 2: Prepara variables de entorno
- Copia `.env.docker.example` a `.env` en la raiz del repo.
- Ajusta `POSTGRES_*`, `DATABASE_URL`, `TELEGRAM_BOT_TOKEN`.
- Ajusta `VITE_API_URL` (debe terminar en `/api`).

Paso 3: Levanta los servicios
- sudo docker compose -f docker-compose.prod.yml --env-file .env up -d --build
- sudo docker ps

Paso 4: Carga datos iniciales (una sola vez)
- sudo docker compose -f docker-compose.prod.yml --env-file .env exec api npm run seed

Paso 5: Configura NPM
- Proxy Host 1: dominio web -> `malulos-web:80`
- Proxy Host 2: dominio API -> `malulos-api:3000`
- Activa SSL y fuerza HTTPS.

Paso 6: Verifica salud basica
- https://TU_DOMINIO
- https://API_DOMINIO/api/health (si existe)

Notas
- Si quieres usar un solo dominio, crea un Custom Location `/api` en NPM que apunte a `malulos-api:3000` y usa `VITE_API_URL=https://TU_DOMINIO/api`.
- Para reiniciar: `sudo docker compose -f docker-compose.prod.yml --env-file .env restart`.
